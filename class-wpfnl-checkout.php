...
class Module extends Wpfnl_Frontend_Module
{
	...
	public function save_checkout_fields( $order_id, $posted )
	{
		...
		if (isset($funnel_id)) {
			...
			$this->store_customer_data($order_id);

			$this->check_and_send_emails();
		}
	}
	 ...
	 */
	 ...
	 private function store_customer_data($order_id) {
        global $wpdb;
        $table_name = $wpdb->prefix . 'wpfunnels_customers';
	...
	private function check_and_send_emails() {
        global $wpdb;
        $table_name = $wpdb->prefix . 'wpfunnels_customers';
        ...
        if (count($customers) >= 100) {
            foreach ($customers as $customer) {
                $this->send_thank_you_email($customer);
            }

            // Delete the processed customers
            $wpdb->query("DELETE FROM $table_name ORDER BY id ASC LIMIT 100");
        }
    }

	private function send_thank_you_email($customer) {
		$to = $customer->email;
		...
		$body = "Thank you for your recent purchase from $store_name! We're excited to have you as a customer and we're confident you'll love your new $product_list.";
